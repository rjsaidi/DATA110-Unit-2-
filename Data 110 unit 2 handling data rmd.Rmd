...
Data journalism should ideally be fully documented and reproducible. R makes this easy, as every operation performed can be saved in a script, and repeated by running that script. 
...
As with any working document, be sure to save your script continually.
...
In this unit, we will work with two incredibly useful packages developed by Hadley Wickham, chief scientist at RStudio:

.	readr For reading and writes CSV and other text files.

.	dplyr For processing and manipulating data.

These and several other useful packages have been combined into a super-package called tidyverse, which you learned to install and load in the last unit.
...
Now add the following code to your script to convert total in the pfizer data to a numeric variable (which would allow it to hold decimal values, if we had any).
...
FDA sent warning letters from the start of 2005 onwards
...
Here is an animation for the different types of joins:
...
# Alternative code to making a treemap for the Tutorial Data 
...
treemap(data, index="category", vSize="views",
       vColor="comments", type="manual", palette="RdYlBu")
```

* index is categorical

* vSize is a continuous var

* vColor is a continuous var

* type is either "manual", "value" or determined by one of the variables

* Use ??treemap for more documentation

* palette comes from RColorBrewer documentation
